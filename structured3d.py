...
class Structured3D(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        background_name = os.path.join(self.root_dir, self.rgb_depth_list[idx][2])
        background = cv2.imread(background_name, -1)
        back_target= background / 255

        camera_name   = os.path.join(self.root_dir, self.rgb_depth_list[idx][4])
        camera_file   = open(camera_name, 'r')
        camera_pose_  = camera_file.readline()
        camera_pose   = camera_pose_.split(' ')
        try:
            camera_z      = float(camera_pose[2])
        except IndexError:
            print("current missed camera_pose file :", camera_z)

        if not self.is_training:
            inputs['cur_name'] = self.rgb_depth_list[idx][0]
        ...
        inputs["background"] = torch.from_numpy(np.expand_dims(back_target, axis=0)).float()
        inputs["camera_z"]   = torch.tensor(camera_z).float()
